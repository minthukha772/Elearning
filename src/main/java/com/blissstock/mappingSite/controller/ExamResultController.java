...
import com.blissstock.mappingSite.model.TestStudentList;
import com.blissstock.mappingSite.model.StudentListForExamineeList;
import com.blissstock.mappingSite.repository.CourseInfoRepository;
import com.blissstock.mappingSite.repository.JoinCourseUserRepository;
import com.blissstock.mappingSite.repository.MarkedAnswerRepository;
import com.blissstock.mappingSite.repository.QuestionRepository;
import com.blissstock.mappingSite.repository.ResultRepository;
//import com.blissstock.mappingSite.service.ResultService;
import com.blissstock.mappingSite.repository.StudentAnswerRepository;
...
import com.blissstock.mappingSite.service.TestStudentService;
...
public class ExamResultController {
    ...
    @Autowired
    private StudentAnswerRepository stuAnsRepo;
    ...
    @Autowired
    private MarkedAnswerRepository markedAnswerRepo;
    ...
    public String getExamResultForStudent(@PathVariable Long testId, @PathVariable Long userId, Model model) {
        ...
        // Result viewExamResult = resultRepo.getResultByTestId(testId);
    ...
    public String getExamineeListForTeacherAndAdmin(@PathVariable Long testId, Model model) {
        // public String getExamineeListForTeacherAndAdmin( Model model) {

        List<TestStudentList> testStudentLists = new ArrayList<>();
        List<StudentListForExamineeList> studentListForExamineeLists = new ArrayList<>();
        List<TestStudent> viewStudents = testStudentRepo.getResultByTestId(testId);
        List<UserAccount> viewAllStudents = userAccountRepo.findByRole("ROLE_STUDENT");
        ...
        if (viewStudents.isEmpty()) {
            ...
            for (TestStudent testStudent : viewStudents) {
                ...
                MarkedAnswer viewMarkedAnswer = markedAnswerRepo.getResultByTestStudentId(testStudentId);
                if (viewMarkedAnswer != null) {
                    ...
                    try {
                        ...
                        testStudentLists.add(new TestStudentList(studentName, studentPhoneNumber, studentEmail,
        ...
        if (!viewAllStudents.isEmpty()) {
            for (UserAccount userAccountList : viewAllStudents) {
                ...
                if (!viewStudents.isEmpty()) {
                    for (TestStudent testStudent : viewStudents) {
                        if (userAccountList.getAccountId().equals(testStudent.getUser().getAccountId())) {
                            userFound = true;
                            break;
                        }
                    }
    ...
    public String addAllEnrolledStudents(@PathVariable Long testId) {
        ...
        List<TestStudent> viewTestStudents = testStudentRepo.getResultByTestId(testId);
            ...
            for (JoinCourseUser joinUser : viewJoinUser) {
                ...
                if (!viewTestStudents.isEmpty()) {
                    ...
                    if (!userFound) {
                        ...
                        // Long newStudentId = joinUser.getUserInfo().getUserAccount().getAccountId();
                        ...
                        System.out.println("Same User from Join Course found in Test Student database.");
                        continue;
                    }

                } else if (joinUser.getUserInfo().getUserAccount().getRole().equals(UserRole.STUDENT.getValue())) {
                    TestStudent newTestStudentId = new TestStudent();
                    // Long newStudentId = joinUser.getUserInfo().getUserAccount().getAccountId();
                    String accountEmail = joinUser.getUserInfo().getUserAccount().getMail();
                    UserAccount user = userAccountRepo.findByMail(accountEmail);
    ...
    public String addCustomStudent(@PathVariable("testId") Long testId, @PathVariable("email") String email) {
    ...
    public String getExamResultListForTeacherAndAdmin(@PathVariable Long testId, Model model) {
        ...
        if (!viewTestStudents.isEmpty()) {
            ...
            double passRate = ((double) passedStudents / (double) totalExamineeList) * 100;
            ...
            for (Question question : viewQuestions) {
                ...
                if (!viewStudentAnswer.isEmpty()) {
                    ...
                    for (StudentAnswer studentAnswer : viewStudentAnswer) {
                        if (question.getQuestionId().equals(studentAnswer.getQuestion().getQuestionId())) {
            ...
            for (TestStudent testStudent : viewTestStudents) {
                ...
                if (viewExamResult != null) {
                    ...
                    try {
                        FileInfo profilePic = storageService.loadProfileAsFileInfo(userInfo);
                        // model.addAttribute("profilePic", profilePic);
                        studentListForExamResults.add(new StudentListForExamResult(studentName, studentEmail,
