...
public interface TestRepository extends JpaRepository<Test, Long> {
         ...
         * @return
         */
        @Query(value = "Select * from test where exam_status = :exam_status and examinee_student_id = :user_id order by test_id desc", nativeQuery = true)
        ...
        @Query(value = "Select * from test where examinee_student_id = :user_id and is_delete = 'true' order by test_id desc", nativeQuery = true)
        public List<Test> getDeletedListByUser(@Param("user_id") Long user_id);

        @Query(value = "Select * from test where course_id = :course_id and examinee_student_id = :user_id order by test_id desc", nativeQuery = true)
        public List<Test> getListByCourseAndUser(@Param("course_id") Long course_id, @Param("user_id") Long user_id);

        @Query(value = "Select * from test where date BETWEEN :fromDate and :toDate and examinee_student_id = :user_id order by test_id desc", nativeQuery = true)
        ...
        // @Query(value = "Select * from test where examinee_student_id = :user_id
